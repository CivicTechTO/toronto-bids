...
- **Two hard limits, both observed live, both load-bearing.** (1) **Respond is disabled the moment a posting closes**, so this reaches only currently-open solicitations (~44) — a *recurring capture, not a backfill*. Whatever closes before a run is gone. (2) The bundle download **hard-stops above 500 MB** as a single zip (>100 MB only warns); events over that are logged and skipped, not silently truncated — per-Part download is the upgrade path, unbuilt until an event needs it (`# ponytail:` in `capture_event`).
