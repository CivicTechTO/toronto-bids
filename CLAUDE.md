...
`store_composite_awards` (part of `tb enrich-titles`) ingests the 2009-2012 composite-report appendices as awards **in their own keyspace**. This is not a linking pass: the City's feed publishes **0 awards for 2009, 1 for 2010 and 12 for 2011**, against the 799 sitting in those reports, so for those years this table *is* the record. 976 rows, $2.1B, and it adds 239 firms to the supplier dimension that won only pre-Ariba (Bondfield Construction: $59.5M, invisible to the archive before this).
...
- **Split awards are skipped, not fused** (50 of ~1,030). One call, several winners, each with its own value section: recording the fused name as one firm invents a supplier and hands it the first winner's money. Detected by enumeration (`1. X 2. Y`), segment markers (`Area "A" – ... Area "B" –`), or a name over 80 chars (unnumbered rosters). **Do not "improve" this by counting legal-form tokens** — it flags 81 rows, nearly all single firms that simply carry two (`Canadian Tire Corporation, Limited`, `A.J. Stone Co. Ltd.`). The threshold stays loose because real firms reach 71 chars via aliases (`d.b.a.`, `operating as`, `C.O.B. as`); tightening it drops as many genuine awards as it catches. Three unmarked pairs survive as single rows — #98.
