...
- **Two hard limits, both observed live, both load-bearing.** (1) **Respond is disabled the moment a posting closes**, so this reaches only currently-open solicitations (~44) — a *recurring capture, not a backfill*. Whatever closes before a run is gone. (2) The bundle download **hard-stops above 500 MB** as a single zip (>100 MB only warns), so an oversized event is captured in **batches** (`sources/ariba_batch.py`, #174) — Ariba's own advice ("select specific items and perform multiple downloads"), driven through the virtualised file picker by `AribaPicker`. Rows are addressed by **outline number**, never DOM index (a fixed ~51 checkboxes slide over ~85 logical rows); each batch writes its `batch-NN.json` sidecar **before** its zip, so the partial directory states what it holds rather than implying it from a count; and the canonical `Doc<n>.zip` is merged **only once every planned row is captured or recorded un-capturable** — an incomplete capture stays in `.partial/` and resumes next run, because `capture_attachments` treats the canonical zip as proof the event is archived forever. Once the posting closes the policy inverts (`finalise_partial`): nothing can be re-fetched, so merge what exists and record the gap durably in `Doc<n>.omitted.json`. Salvage fires only on a **stably** disabled Respond, and only when partials exist — the preview renders buttons disabled while it loads, and one spurious read would canonicalise an incomplete capture permanently.
- **Two halves split like fetch/normalize.** The **pure** half (`document_number_from_zip_name`, `index_zip`, `store_bundle`, `ingest_downloads`, `reindex_bundles`) indexes a bundle and writes the manifest — no browser, tested against fixture zips. `index_zip` reads `file_size` and `crc32` **from the zip central directory**, not decompressing the top-level bundle (a nested zip *is* read to reach its own directory — #123). The **browser** half (`login`, `capture_event`, `capture_attachments`) needs a headed Chromium behind the `council` extra, logged into a supplier account from `scrapers/.env` (`ARIBA_USERNAME`/`ARIBA_PASSWORD`, gitignored — the repo is public). The flow is Discovery preview → Respond → **Download Content → Download Attachments → select-all → download**, and the download is one **server-zipped bundle per event**, not per-file `<a href>`s.
