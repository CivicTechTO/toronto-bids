...
- **Pre-Ariba council items** (`match_pre_ariba_titles`, #77) — 2017-2018 agendas name no document number (Toronto adopted Ariba ~2019), so they are matched on **(supplier, award value)** instead. Council publishes three figures per award; `award_amount` is the **"net of all applicable taxes"** one — calibrated against 980 Ariba-era items where the document number gives ground truth (820 matched; "including HST" matched 0). Only a *unique* match is taken: 21 of 5,443 title-less awards share a (supplier, amount) with another document, and a wrong title is worse than none.
