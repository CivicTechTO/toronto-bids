...
The bidders did not stop. `tb enrich-awards --download` archives the Toronto Bids Portal's **Award Summary Form** per award — 617 bids so far, and richer than the agendas ever were.
...
- **`download_pdf(layout=True)` is required here and only here.** Without `-layout` the form's `Label    value` columns collapse and it parses to **nothing at all**. Council PDFs stay on plain extraction: #96 measured the two as identical there (244/280 blocks either way), so re-extracting them differently would rewrite stored text for no gain.
- **The count check is one-sided on purpose.** `Number of Bids Received` catches an under-parse (the agenda corpus never offered that check), but parsing *more* is kept: the count sometimes covers only the **compliant** bids while the table lists everyone (doc `5247418372` declares 2, tabulates 3, third marked `*` at $0.00). The table is the record. 11 forms (4%) are refused and the count is printed, never silent.
- **Three traps this parser hit, all previously documented elsewhere:** an RFP lists proponents with **no price at all** (`NOTE: Not applicable for RFP`) so the price must be optional (#84 stores these as NULL); `2489960 Ontario Inc.` is a **real firm**, so a "long digit run means a leaked price" guard eats it (#87 pins the same lesson); and `\s` matches newlines, so `\d{1,2}[.)]\s*` walked off an empty `4.` row and captured `Page 2 of 2` as a bidder.
