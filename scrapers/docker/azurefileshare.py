...
class AzureFileShare:
    def __init__(self, keychain: Keychain, share_name: str):
        self.service = ShareServiceClient.from_connection_string(
            f"DefaultEndpointsProtocol=https;AccountName=torontobidsstorage;AccountKey={keychain.get_secret('AZURESTORAGEKEY')};EndpointSuffix=core.windows.net"
        )
    ...
    def create_file_listing_dataframe(self) -> pd.DataFrame:
        ...
        download_links = [self.generate_download_link(file_path) for file_name, file_path in files]
