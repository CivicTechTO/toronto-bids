...
def get_latest_open_data() -> pd.DataFrame:
    ...
    try:
        data["ShowDateClosing"] = pd.to_datetime(data["ShowDateClosing"], format="%B %d, %Y")
        data["ShowDatePosted"] = pd.to_datetime(data["ShowDatePosted"], format="%B %d, %Y")
    except ValueError:
        # If that format failed, it's going to be like [<font color=red><b>May 30, 2023 12:00 noon</b>]
        data["ShowDateClosing"] = pd.to_datetime(data["ShowDateClosing"].str.extract(r"(\w+ \d+, \d+)", expand=False),
                                                 format="%B %d, %Y")
        data["ShowDatePosted"] = pd.to_datetime(data["ShowDatePosted"].str.extract(r"(\w+ \d+, \d+)", expand=False),
                                                format="%B %d, %Y")
