...
- **Council enrichment** (`tb enrich-council`, OPT-IN) — for each suspended firm, fetches its
  ...
  of `tb sync`, but Playwright, `pyvirtualdisplay`, and `python-dotenv` are **mandatory base
  deps** (installed by a plain `uv sync`) — the nightly runs the Ariba-attachment and Zoo/EP
  browser captures in-line, so they can't be optional. Only the browser binary is separate:
  `uv run playwright install chromium`. On a headless server run `tb enrich-council
  --virtual-display` with `Xvfb` installed (`apt-get install -y xvfb`); `pdftotext` (poppler)
  is also required.
