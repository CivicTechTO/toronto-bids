"""EP board report infrastructure tests (#130).

Parser tests were removed in #205 (LLM extraction replaces per-source parsers).
These test the term discovery and buyer seeding infrastructure that remains.
"""
...
def test_ep_terms_probe_the_ep_committee_series():
    ...
    found = discover_meetings(
        fetch, term_starts=[("EP", 2023, "2022-2026", 1)], stop_after_misses=2
    )
    assert found == {}
...
def test_ep_buyer_seeded():
    ...
    row = conn.execute(
        "SELECT kind, partnered FROM buyer WHERE slug='exhibition-place'"
    ).fetchone()
