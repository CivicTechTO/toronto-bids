"""Zoo board report infrastructure tests (#135).

Parser tests were removed in #205 (LLM extraction replaces per-source parsers).
These test the term discovery infrastructure that remains.
"""
...
def test_discover_meetings_accepts_custom_term_starts():
    ...
    found = discover_meetings(
        fetch, term_starts=[("ZB", 2019, "2018-2022", 1)], stop_after_misses=2
    )
    assert found == {}
