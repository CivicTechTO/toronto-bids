...
def process_folder(folder_name, CallNumber, subfolder=0):
    ...
    this_folder = ShareDirectoryClient.from_connection_string(
        conn_str=connection_string,
        share_name="torontobids",
        directory_path=folderbase + "/" + folder_name,
    )
    this_folder_files_list = list(this_folder.list_directories_and_files())
    attachments = (
        {}
    )  # reset the dict of attachments, since we're at the top of a new folder
    jsonfiles = []

    for this_file in this_folder_files_list:
        if this_file["is_directory"]:
            foldertext += process_folder(
                folder_name + "/" + this_file["name"], CallNumber, 1
            )
            ...
            print(
                "parsing: " + folderbase + "/" + folder_name + "/" + this_file["name"]
            )
            ...
            if read_pdfs:
                if this_file["name"][-4:].lower() == ".pdf":
                    ...
                    pdf_file_client = ShareFileClient.from_connection_string(
                        conn_str=connection_string,
                        share_name="torontobids",
                        file_path=folderbase
                        + "/"
                        + folder_name
                        + "/"
                        + this_file["name"],
    ...
    if subfolder == 0:
        if jsonfiles and jsonfiles[0]:
            ...
            file_client = ShareFileClient.from_connection_string(
                conn_str=connection_string,
                share_name="torontobids",
                file_path=folderbase + "/" + folder_name + "/" + jsonfiles[0],
            )
            ...
            sqlreal = (
                "INSERT INTO calls (Commodity,CommodityType,CallNumber,Type,ShortDescription,Description,ShowDatePosted,ClosingDate,SiteMeeting,ShowBuyerNameList,BuyerPhoneShow,BuyerEmailShow,Division,BuyerLocationShow,parsedtext,uuid) "
...
parent_dir = ShareDirectoryClient.from_connection_string(
    conn_str=connection_string, share_name="torontobids", directory_path=folderbase
)
all_folders = list(parent_dir.list_directories_and_files())

for this_folder_data in all_folders:
    if (
        (this_folder_data["name"] >= folder_to_start_with)
        and (this_folder_data["name"] not in folders_to_skip)
        and (this_folder_data["is_directory"])
    ):
